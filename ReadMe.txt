...
main() will create a list of element names fom the PyQt6_ui.py ('uifile') file chosen by the user.  It will: 
    1.) Search the uifile for element names by finding the occurrences of the "setObjectName()" method.  These are
        the object names created either Qt Designer or by the coder of the uifile.  
    2.) Create a list of the names of of those elements.
    3.) Uses that list crete a second list by searching the uifile for references to where those elements were set to objects by finding 
